...
def build_final_benchmark_matrix() -> Dict[str, Any]:
    ...
    centralized_data = load_json(CENTRALIZED_PATH) or {}
    ...
    # Centralized rows
    for model_name, mdata in centralized_data.items():
        val = mdata.get("validation", {})
        test = mdata.get("test", {})
        rows.append({
            "Experiment": f"Centralized {model_name.upper()}",
            ...
            "SecAgg": "N/A",
            "Attack": "None",
            "Rounds": 50 if "mlp" in model_name.lower() else "N/A",
            "Best Round": mdata.get("best_epoch", "N/A"),
            ...
            "Train Time (s)": round(mdata.get("train_time_seconds", 0.0), 1),
        })
